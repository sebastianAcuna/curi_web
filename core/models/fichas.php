<?php
    ...
    class Fichas{
        ...
        public function set_numAnexo($numAnexo){
            $numAnexo = (isset($numAnexo)?$numAnexo:NULL);
            $this->numAnexo = filter_var($numAnexo,FILTER_SANITIZE_STRING);
        ...
        public function traerDatosActivas(){
            try{
                ...
                $having = "";
                if($this->fieldbook != ""){ $having = ($this->fieldbook == 0)?"HAVING fielbook IS NULL":"HAVING fielbook != 'NULL'"; }

                if($this->fieldman != ""){ $filtro .= " AND CONCAT(U.nombre,' ',U.apellido_p,' ',U.apellido_m) LIKE '%$this->fieldman%'"; }
                if($this->temporada != ""){ $filtro .= " AND T.nombre LIKE '%$this->temporada%'"; }
                if($this->especie != ""){ $filtro .= " AND F.id_esp = '$this->especie'"; }
                ...
                if($this->region != ""){ $filtro .= " AND F.id_region LIKE '$this->region'"; }
                if($this->comuna != ""){ $filtro .= " AND F.id_comuna LIKE '$this->comuna'"; }
                ...
                if($this->suelo != ""){ $filtro .= " AND F.id_tipo_suelo = '$this->suelo'"; }
                if($this->riego != ""){ $filtro .= " AND F.id_tipo_riego = '$this->riego'"; }
                if($this->experiencia != ""){ $filtro .= " AND F.experiencia = '$this->experiencia'"; }
                if($this->tenencia != ""){ $filtro .= " AND F.id_tipo_tenencia_terreno = '$this->tenencia'"; }
                if($this->maquinaria != ""){ $filtro .= " AND F.id_tipo_tenencia_maquinaria = '$this->maquinaria'"; }
                ...
                if($this->numAnexo != ""){ $filtro .= " AND AC.num_anexo LIKE '%$this->numAnexo%'"; }
                if($this->ficha != ""){ $filtro .= " AND F.id_ficha = '$this->ficha'"; }
                ...
                switch($this->orden){
                    ...
                    break;
                    case 71:
                        $orden = "ORDER BY AC.num_anexo ASC";
                    break;
                    case 72:
                        $orden = "ORDER BY AC.num_anexo DESC";
                    break;
                    case 73:
                ...
                $sql = "SELECT AC.num_anexo, (SELECT id_ac FROM anexo_contrato INNER JOIN visita USING(id_ac) WHERE anexo_contrato.id_ficha = F.id_ficha GROUP BY id_ac LIMIT 1) AS fielbook, F.id_ficha, CONCAT(U.nombre, ' ', U.apellido_p, ' ', U.apellido_m) AS fieldman, T.nombre AS temporada, E.nombre AS especie, A.rut, A.razon_social, A.telefono, L.nombre_ac, L.telefono_ac, F.oferta_de_negocio, F.localidad, R.nombre AS region , C.nombre AS comuna, F.ha_disponibles, A.direccion, A.rep_legal, A.rut_rl, A.telefono_rl, A.email_rl, A.banco, A.cuenta_corriente, P.nombre AS predio, L.nombre AS potrero, group_concat(CASE WHEN H.tipo = 'F' THEN CONCAT(H.anno,' => ',H.descripcion) END ORDER BY H.anno ASC) AS rotacion, L.coo_utm_ampros, L.radio, TR.descripcion AS riego, TS.descripcion AS suelo, F.experiencia, TT.descripcion AS tenencia, TM.descripcion AS maquinaria, F.maleza, F.estado_general, F.obs 
                        FROM ficha F 
                        ...
                        $filtro GROUP BY F.id_ficha $having $orden LIMIT $this->desde,10";
        ...
        public function totalDatosActivas(){
            try{
                ...
                $conexion = new Conectar();
                $sql = "SELECT (SELECT id_ac FROM anexo_contrato INNER JOIN visita USING(id_ac) WHERE anexo_contrato.id_ficha = F.id_ficha GROUP BY id_ac LIMIT 1) AS fielbook, F.id_ficha FROM ficha F  
                        INNER JOIN usuarios U ON U.id_usuario = F.id_usuario 
                        INNER JOIN especie E ON E.id_esp = F.id_esp 
                        INNER JOIN agricultor A ON A.id_agric = F.id_agric 
                        INNER JOIN region R ON R.id_region = F.id_region 
                        ...
                        LEFT JOIN tipo_tenencia_terreno TT ON TT.id_tipo_tenencia_terreno = F.id_tipo_tenencia_terreno 
                        LEFT JOIN tipo_tenencia_maquinaria TM ON TM.id_tipo_tenencia_maquinaria = F.id_tipo_tenencia_maquinaria 
                        $filtro GROUP BY F.id_ficha $having";
        ...
        public function editarFichaA(){
            try{
                ...
                if($consulta->rowCount() > 0 && ($consulta->rowCount() == $consulta1->rowCount())){
                    $cont++;
                    for($i = 0; $i < 4; $i++){
                        $sql = "UPDATE historial_predio SET descripcion = ? WHERE anno = ? AND id_ficha = ? AND tipo = 'F'";
                        $consulta = $conexion->prepare($sql);
                        switch($i){
                            case 0:
                    ...
                    $contS++;
                    for($i = 0; $i < 4; $i++){
                        $sql = "SELECT * FROM historial_predio WHERE descripcion = ? AND anno = ? AND id_ficha = ? AND tipo = 'F'";
                        $consulta = $conexion->prepare($sql);
                        $consulta->bindValue("1",$this->id, PDO::PARAM_INT);
                        switch($i){
                            case 0:
        ...
        public function traerRotacion(){
            try{
                ...
                $conexion = new Conectar();
                $sql = "SELECT anno, descripcion FROM historial_predio WHERE id_ficha = ? AND tipo = 'F' ORDER BY anno DESC LIMIT 4";
                $conexion = $conexion->conexion();
                $consulta = $conexion->prepare($sql);
                $consulta->bindValue("1",$this->id, PDO::PARAM_INT);
                $consulta->execute();
        ...
        public function activarFicha(){
            ...
            try{
                ...
                if($consulta->rowCount() > 0 ){
                    ...
                    for($i = 0; $i < 4; $i++){
                        $sql = "INSERT INTO historial_predio (id_ficha, anno, descripcion, tipo) VALUES (?, ?, ?, 'F')";
        ...
        public function eliminarImagen(){
            ...
            try{
                ...
                echo "[ELIMINAR IMAGENES] -> ha ocurrido un error ".$e->getMessage();
